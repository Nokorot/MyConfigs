...
if [ -f "${cached::-4}.txc" ] && [ "${cached::-4}.txc" -nt "$path" ]; then
	try cat "${cached::-4}.txc" && { dump | trim; exit 0; }
fi
...
case "$extension" in
	# Cached text files (To avoid recursive caching): 
	txc)
...
case "$mimetype" in
    ...
    video/* | audio/*)
        ...
        try ffmpegthumbnailer -t 5% -s 512 -i "$path" -o "${cached//.png}" && exit 6;
